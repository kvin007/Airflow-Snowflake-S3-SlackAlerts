...
-- Create the schema
CREATE OR REPLACE SCHEMA DB.MEETUP;

-- Create the file format for the csv files
CREATE OR REPLACE FILE FORMAT DB.MEETUP.FORMAT_TEST 
TYPE=CSV
...
-- Categories;
CREATE OR REPLACE TABLE DB.MEETUP.categories (
category_id       int,
category_name    text,
shortname        text,
sort_name        text
...
copy into DB.MEETUP.categories(category_id, category_name, shortname, sort_name)
from (
  select t.$1,t.$2,t.$3, t.$4
  from @csvfiles/categories.csv.gz t
)
file_format = (format_name = DB.MEETUP.FORMAT_TEST);

-- Cities;
CREATE OR REPLACE TABLE DB.MEETUP.cities(
city                       text,
city_id                     int,
country                    varchar(2),
distance                  numeric,
...
copy into DB.MEETUP.cities(city, city_id, country, distance, latitude,localized_country_name, longitude, member_count,ranking, state, zip)
from (
...
-- Events
CREATE OR REPLACE TABLE DB.MEETUP.events
(
	event_id                         VARCHAR(12),
	created                          datetime,
	description                      text,
...
copy into DB.MEETUP.events(event_id, created, description, duration, event_url,
...
-- Groups
CREATE OR REPLACE TABLE DB.MEETUP.groups
(
	group_id                          bigint,
	category_id                       int,
	"category.name"                    text,
...
copy into DB.MEETUP.groups(group_id,category_id,"category.name","category.shortname",city_id,
...
-- Groups Topics;
CREATE OR REPLACE TABLE DB.MEETUP.groups_topics(
topic_id       bigint,
topic_key     text,
topic_name    text,
group_id       bigint
...
copy into DB.MEETUP.groups_topics(topic_id, topic_key, topic_name, group_id)
from (
  select t.$1,t.$2,t.$3,t.$4
  from @csvfiles/groups_topics.csv.gz t
)
file_format = (format_name = DB.MEETUP.FORMAT_TEST, encoding= 'ISO88591');


-- Members;
CREATE OR REPLACE TABLE DB.MEETUP.members(
member_id          int,
bio               text,
city              varchar(100),
country           varchar(2),
...
copy into DB.MEETUP.members(member_id, bio, city, country, hometown, joined, lat,
...
-- Members Topics;
CREATE OR REPLACE TABLE DB.MEETUP.members_topics(
topic_id       bigint,
topic_key     text,
topic_name    text,
group_id       bigint
...
copy into DB.MEETUP.members_topics(topic_id, topic_key, topic_name, group_id)
from (
  select t.$1,t.$2,t.$3,t.$4
...
-- Topics;
CREATE OR REPLACE TABLE DB.MEETUP.topics(
topic_id          bigint,
description      text,
link             text,
members           bigint,
...
copy into DB.MEETUP.topics(topic_id, description, link, members, topic_name, urlkey, main_topic_id)
from (
...
-- Venues;
CREATE OR REPLACE TABLE DB.MEETUP.venues(
venue_id                    bigint,
address_1                  text,
city                       varchar(100),
country                    varchar(3),
...
copy into DB.MEETUP.venues(venue_id, address_1, city, country, distance, lat,
